...
void translate(screen_char_t *s, NSUInteger len) {
    int i;
    for (i = 0; i < len; i++) {
...
void padString(NSString *s, screen_char_t *buf, int fg, int bg, NSUInteger *len,
...
- (void)putToken:(VT100TCC)token {
    ...
    switch (token.type) {
        ...
        case VT100CSI_EL:
            ...
            break;
        case VT100CSI_HTS:
            if (CURSOR_X < WIDTH) tabStop[CURSOR_X] = YES;
            break;
        case VT100CSI_HVP:
            [self cursorToX:token.u.csi.p[1] Y:token.u.csi.p[0]];
            break;
        ...
        case VT100CSI_TBC:
            switch (token.u.csi.p[0]) {
                case 3:
                    ...
                    break;
                case 0:
                    if (CURSOR_X < WIDTH) tabStop[CURSOR_X] = NO;
            }
            break;
...
- (void)saveBuffer {
    ...
    size_t n = (screen_top - buffer_lines) / REAL_WIDTH - max_scrollback_lines;
